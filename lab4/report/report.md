...
- `calcSpillCost` 函数的执行流程？
    ...
    - 如果此寄存器没有被 disabled（此时可以被分配），则根据它的状态返回代价
        ...
        - 如果是其它，找到和它对应的虚拟寄存器，根据它是否 dirty（是否需要 spill）确定代价。
...
不同点：
- 龙书中 R 的「得分」为需要生成的保存指令的个数。而 LLVM 中对应的概念为「`Cost`」，它不是需要生成的保存指令的个数；
- LLVM 在正式分配寄存器前先尝试两个 hint。

对「得分」的定义不同表现为 LLVM 中对空寄存器、不空但是不需要 spill 的寄存器和不能被分配的寄存器都进行了评分，因为这样每个寄存器都可以有一个得分，在实现上选择寄存器时更加方便。还有 LLVM 处理了龙书中没有讲到的重名寄存器的情形。

在分配寄存器前先尝试 hint 是为了让寄存器分配更快和更优，特别是涉及到 copy 类的指令时。
...
## 实验总结
- 了解了 Makefile 中的 `.SUFFIXES` 和 `.SECONDARY` 两个 rule；
